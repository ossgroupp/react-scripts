...
const { logInfo, initialiseRepository } = require('@osspim/cli-utils');
...
const configureTemplate = (projectPath, options) => {
  ...
  // fs.removeSync(path.resolve('.osspim-greeting'));
...
const configureEnvironment = async (projectPath, options) => {
  ...
  const envVars = {
    NEXT_PUBLIC_OSSPIM_INSTANCE_IDENTIFIER: options.instanceId,
    JWT_SECRET: 'come-up-with-a-super-secret-token-here'
  };

  const envLocalVars = {
    OSSPIM_SECRET_TOKEN_ID: options.osspimAccessTokenId,
    OSSPIM_SECRET_TOKEN: options.osspimAccessTokenSecret
  };
